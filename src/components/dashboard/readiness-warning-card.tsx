...
export function ReadinessWarningCard({ readiness, moduleName = "Forecasting" }: ReadinessWarningCardProps) {
  ...
  return (
    <div className={cn("rounded-2xl border bg-white/60 dark:bg-slate-900/60 p-6 lg:p-8 shadow-xl max-w-[900px] mx-auto space-y-6", borderCls)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white font-space-grotesk">{moduleName} Service Locked</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Current Dates */}
        <div className="bg-slate-100/60 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-850 p-4 rounded-xl text-center">
        ...
        {/* Minimum Required */}
        <div className="bg-slate-50/40 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-900 p-4 rounded-xl text-center">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Minimum Required</span>
          <span className="text-2xl font-black font-space-grotesk text-slate-700 dark:text-slate-300 mt-1 block">30</span>
        ...
        <div className="bg-slate-50/40 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-900 p-4 rounded-xl text-center">
          ...
          <span className="text-2xl font-black font-space-grotesk text-slate-700 dark:text-slate-300 mt-1 block">180+</span>
          <span className="text-xs text-slate-500 mt-1 block">dates for full seasonality checks</span>
        </div>
      </div>

      {/* Explanation text */}
      <div className="space-y-2 bg-slate-50/40 dark:bg-slate-950/40 p-4 rounded-xl border border-slate-200 dark:border-slate-900 text-sm">
        ...
        <p className="text-slate-400 dark:text-slate-500 leading-relaxed text-[11px] pt-1.5 border-t border-slate-200 dark:border-slate-900">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div 
              key={index} 
              className={cn(
                "bg-slate-100/60 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-900 p-3.5 rounded-xl flex items-center gap-3",
