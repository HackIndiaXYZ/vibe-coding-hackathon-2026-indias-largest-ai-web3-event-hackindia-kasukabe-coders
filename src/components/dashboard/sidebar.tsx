    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-5 border-b border-slate-200 dark:border-slate-800">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-base font-bold leading-none font-space-grotesk">
                <span className="text-slate-900 dark:text-white">Mandi</span>
          ...
          <button
            onClick={onClose}
            className="lg:hidden text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors p-1"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-5 py-3">
          <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 text-xs font-medium">Live Data — 2,847 Mandis</span>
          </div>
        </div>

        {/* Nav */}
        <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
          <div className="text-[10px] font-semibold text-slate-400 dark:text-slate-600 uppercase tracking-wider px-3 py-2">
            Navigation
          </div>
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ...
                      : item.isNew
                        ? "text-violet-500 dark:text-violet-400 hover:text-violet-600 dark:hover:text-violet-300 hover:bg-violet-500/10 border border-violet-500/10"
                ...
                >
                  <item.icon
                    className={cn(
                      ...
                      active
                        ? item.isNew ? "text-violet-500 dark:text-violet-400" : "text-emerald-500 dark:text-emerald-400"
                        : item.isNew
                  ...
                  {active && (
                    <ChevronRight className={cn("w-4 h-4", item.isNew ? "text-violet-500 dark:text-violet-400" : "text-emerald-500 dark:text-emerald-400")} />
                  )}
                </Link>
              </div>
            );
        ...
        <div className="px-4 py-4 border-t border-slate-200 dark:border-slate-800">
          <div className="bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
            <div className="text-xs font-semibold text-slate-900 dark:text-white mb-1">Data last updated</div>
            <div className="text-xs text-slate-500 dark:text-slate-400">Jun 1, 2024 · 2:45 PM IST</div>
            <div className="mt-2 h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-emerald-500 to-green-400 rounded-full w-4/5" />
            </div>
            <div className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Next refresh in 15 min</div>
          </div>
        </div>
      </aside>
    </>
